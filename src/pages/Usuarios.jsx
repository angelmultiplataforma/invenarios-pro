...
export function Usuarios() {
  ...
  const { isLoading, error } = useQuery({
    queryKey: ["mostrar usuarios", { _id_empresa: dataempresa.empresa?.id }],
  ...
  const { data: buscardata } = useQuery({
    queryKey: [
      "buscar usuarios",
      { _id_empresa: dataempresa.empresa?.id, buscador: buscador },
    ],
    queryFn: () =>
      buscarusuarios({ _id_empresa: dataempresa.empresa?.id, buscador: buscador }),
